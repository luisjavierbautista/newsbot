...
export default function Facts() {
  ...
  const { data, isLoading, refetch, isFetching } = useQuery({
    ...
    staleTime: 5 * 60 * 1000, // 5 minutes - check cache frequently but it's cheap
    ...
    <div className="min-h-screen pb-6 sm:pb-8 overflow-x-hidden">
        ...
        <div className="relative">
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3">
            ...
            <button
              onClick={() => refetch()}
              disabled={isFetching}
              className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 bg-dark-700 hover:bg-dark-600 disabled:opacity-50 text-white rounded-lg sm:rounded-xl text-xs sm:text-sm font-medium transition-colors"
              title="Recargar datos en cache"
            >
          ...
          {data?.cached && (
            <div className="mt-3 text-xs text-gray-500 flex items-center gap-2">
              ...
              <span>
                Actualizado hace {data.cache_age_hours ? `${data.cache_age_hours}h` : 'poco'}
                {data.is_stale && ' (actualizando pronto...)'}
              </span>
            </div>
          )}
        </div>
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-20">
          ...
          <p className="text-gray-400 mt-4">Cargando hechos...</p>
        </div>
      ) : facts.length === 0 ? (
        <div className="text-center py-20">
          <Sparkles className="w-12 h-12 text-gray-600 mx-auto mb-4" />
          <p className="text-gray-400 text-lg">
            {data?.message || 'No hay datos disponibles para este periodo'}
          </p>
          <p className="text-gray-500 text-sm mt-2">
            Los datos se actualizan automaticamente cada 2 horas.
            <br />
            Usa las fechas por defecto (ayer a hoy) para ver los hechos mas recientes.
          </p>
        </div>
      ) : (
        <>
